...
const generateTxSig = async (
  ...
  try {
    ...
    if (_retryCount >= MAX_RETRIES) {
      throw new Error(`generateTxSig failed after ${MAX_RETRIES} retries`);
    }
    return generateTxSig(email, upAuthToken, action, keystore, localKeyAddress, password, rawData, _retryCount + 1);
  }
};

const generateLocalKey = async (data: StartKeyGenInput, password: string, _retryCount = 0): Promise<LocalKeyData | null> => {
  try {
    ...
    if (_retryCount >= MAX_RETRIES) {
      throw new Error(`generateLocalKey failed after ${MAX_RETRIES} retries`);
    }
    return generateLocalKey(data, password, _retryCount + 1);
  }
};
