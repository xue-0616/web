...
const generateLocalKey = async (data: StartKeyGenInput, _retryCount = 0): Promise<LocalKeyData | null> => {
  try {
    ...
    return generateLocalKey(data, _retryCount + 1)
  }
}
...
const generateTxSig = async (
  ...
  try {
    ...
    return generateTxSig(localKey, localKeyAddress, rawData, _retryCount + 1)
  }
}
