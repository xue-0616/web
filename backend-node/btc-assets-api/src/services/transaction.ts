...
export default class TransactionProcessor implements ITransactionProcessor {
  ...
  private async appendPaymasterCellAndSignTx(
    ...
    // BA-H2 FIX: Pass JWT subject for per-address rate limiting
    const tx = await this.cradle.paymaster.appendCellAndSignTx(btcTx.txid, {
      ...ckbVirtualResult,
      ckbRawTx: signedTx!,
    }, (ckbVirtualResult as any)?.context?.jwt?.sub);
    return tx;
  }
   ...
   * BA-H1 FIX: Add deduplication check to prevent double-submission of the same btc_txid
