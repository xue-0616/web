import { BI, Cell, config, hd, helpers } from '@ckb-lumos/lumos';
...
export default class Paymaster implements IPaymaster {
  ...
  /**
   * BA-H2 FIX: Check per-address daily limit on paymaster cell consumption
   * Default: max 50 paymaster cells per JWT subject per day
   */
  private static readonly PAYMASTER_DAILY_LIMIT = 50;

  public async checkPaymasterRateLimit(jwtSubject?: string): Promise<void> {
    if (!jwtSubject) return; // If no JWT, skip (should be caught by auth middleware)
    const key = `rgbpp:paymaster:rate:${jwtSubject}`;
    const currentCount = await this.cradle.redis.incr(key);
    if (currentCount === 1) {
      await this.cradle.redis.expire(key, 86400); // 24 hours
    }
    if (currentCount > Paymaster.PAYMASTER_DAILY_LIMIT) {
      this.cradle.logger.warn(`[Paymaster] Daily rate limit exceeded for ${jwtSubject}: ${currentCount}`);
      throw new PaymasterCellNotEnoughError(`Daily paymaster cell limit exceeded (max ${Paymaster.PAYMASTER_DAILY_LIMIT})`);
    }
  }
   ...
   * @param jwtSubject - optional JWT subject for rate limiting
   */
  public async appendCellAndSignTx(
    ...
    try {
      // BA-H2 FIX: Enforce per-address daily limit on paymaster usage
      await this.checkPaymasterRateLimit(jwtSubject);
