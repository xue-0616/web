...
export class SysTaskController {
    ...
    async page(@Query() dto: PageOptionsDto): Promise<PaginatedResponseDto<SysTask>> {
        ...
        return { list, pagination: { total: count, size: dto.limit, page: dto.page } };
    }
    ...
    async once(@Body() dto: CheckIdTaskDto): Promise<void> {
        ...
        if (!isEmpty(task)) await this.taskService.once(task);
        else throw new ApiException(10020);
    }
    ...
    async stop(@Body() dto: CheckIdTaskDto): Promise<void> {
        const task = await this.taskService.info(dto.id);
        if (!isEmpty(task)) await this.taskService.stop(task);
        else throw new ApiException(10020);
    }
    ...
    async start(@Body() dto: CheckIdTaskDto): Promise<void> {
        const task = await this.taskService.info(dto.id);
        if (!isEmpty(task)) await this.taskService.start(task);
        else throw new ApiException(10020);
    }
    ...
    async delete(@Body() dto: CheckIdTaskDto): Promise<void> {
        const task = await this.taskService.info(dto.id);
        if (!isEmpty(task)) await this.taskService.delete(task);
        else throw new ApiException(10020);
    }
}
