...
export class SysUserController {
    ...
    @ApiOperation({ summary: '查询管理员信息' })
    @ApiOkResponse({ type: UserDetailInfo })
    @Get('info')
    async info(@Query() dto: InfoUserDto): Promise<UserDetailInfo> {
        return this.userService.info(dto.userId);
    }

    @ApiOperation({ summary: '根据ID列表删除管理员' })
    @Post('delete')
    ...
    @Post('page')
    async page(@Body() dto: PageSearchUserDto, @AdminUser() user: IAdminUser): Promise<PaginatedResponseDto<PageSearchUserInfo>> {
        const [list, total] = await this.userService.page(user.uid, dto);
        return { list, pagination: { total, page: dto.page, size: dto.limit } };
    }
